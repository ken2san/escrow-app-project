...
const ProposalItem = ({ proposal, lang, t, isAnyProposalSelectedOnProject, onViewDetails }) => {
  return (
    ...
    <div className="text-xs text-gray-600 mt-2">
      <span>
        {t('proposedAmount')}: {proposal.proposedAmount ? `¥${Number(proposal.proposedAmount).toLocaleString()}` : proposal.desiredAmount ? `¥${Number(proposal.desiredAmount).toLocaleString()}` : '-'}
      </span>
